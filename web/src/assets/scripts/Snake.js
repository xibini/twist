...
export class Snake extends AcGameObject {
    constructor(info, gamemap) {
        ...
        if (this.id === 1) this.eye_direction = 2; // 初始蛇头的方向
        this.eye_dx = [ // 四个方向上蛇在x轴上的偏移量
            [-1, 1],
            [1, 1],
            [1, -1],
            [-1, -1],
    ...
    check_tail_increasing() { // 检测当前回合蛇尾是否增加
        if(this.step <= 10) return true; // 十回合内加一
        if(this.step % 3 === 1) return true; // 十回合之后每三步加一
        return false;
    }
    ...
    update_move() {
        ...
        if (distance < this.eps) {  // 走到目标点了
            ...
            if(!this.check_tail_increasing()) { // 如果蛇尾不变长，蛇尾走向下一个目的地
                const k = this.cells.length;
